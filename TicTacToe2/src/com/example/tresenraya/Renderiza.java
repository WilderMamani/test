...
public class Renderiza extends GLSurfaceView implements Renderer {
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig arg1) {
		...
		cruz = new cruz();
		ray = new ray();
		gl.glClearColor(0f / 240, 255f / 255, 255f / 255, 0); // color de fondo
	}
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		if (e.getAction() == MotionEvent.ACTION_UP) {
			...
			if (fin == 0) {
				...
				if (verificaGana() == 1) {
					Toast.makeText(context.getApplicationContext(), "REDONDOS, GANASTE", Toast.LENGTH_LONG).show();
					fin=1;
				}

				if (verificaGana() == 2) {
					Toast.makeText(context.getApplicationContext(), "X GANO, PERDISTE", Toast.LENGTH_LONG).show();
					fin=1;
				}
				if (verificaGana() == 0) {
...
//			int ny = (int) Math.floor(Math.random()*3);
	...
	private int fichaGanadora(){
		int cont = 0, res = 0;
		for(int i = 0; i< 3; i++){
			if((m[i][0]+m[i][1]+m[i][2])==4&&(m[i][1]!=1&&m[i][2]!=1)){
				for(int j = 0; j< 3; j++){
					m[i][j] = 2;
				}
				res = 1;
		...
		if(res == 0){
			for(int i = 0; i< 3; i++){
				if((m[0][i]+m[1][i]+m[2][i])==4&&(m[1][i]!=1&&m[2][i]!=1)){
					for(int j = 0; j< 3; j++){
						m[j][i] = 2;
					}
					res = 1;
	...
	private int evitarVictoria(){
		int cont = 0, res = 0;
		for(int i = 0; i< 3; i++){
			if((m[i][0]+m[i][1]+m[i][2])==2&&(m[i][0]!=2&&m[i][1]!=2&&m[i][2]!=2)){
				for(int j = 0; j< 3; j++){
					if(m[i][j]!=1 && res == 0){
						m[i][j] = 2;
						res = 1;
		...
		if(res == 0){
			for(int i = 0; i< 3; i++){
				if((m[0][i]+m[1][i]+m[2][i])==2&&(m[0][i]!=2&&m[1][i]!=2&&m[2][i]!=2)){
					for(int j = 0; j< 3; j++){
						if(m[j][i]!=1 && res == 0){
							m[j][i] = 2;
							res = 1;
